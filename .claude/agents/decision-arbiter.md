...
4. Synthesize, then compute `qty`. Python creates the live stop-loss order using your `stop_loss` only when it is tighter than the 1% floor.

# Sizing math you MUST respect

- `qty * entry_price <= equity * max_position_pct` (default 50%).
- Python enforces a stop-market order no wider than 1% below entry on every BUY/ADD. You may provide a tighter `stop_loss`; wider stops are rejected. Use `qty * entry_price * hard_stop_loss_pct` as the default trade risk unless you supply a tighter stop. If your conviction-driven weight would breach max risk, REDUCE qty.
...
- `stop_loss` and `take_profit` are optional and may be null. If `stop_loss` is supplied, it must be tighter than 1% below `entry_price`; otherwise Python rejects the trade.
